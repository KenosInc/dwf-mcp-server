...
dwfpy <= 1.2.0 declares the rgRX parameter of FDwfDigitalSpiWriteRead (and the
16-/32-bit variants) as ``_OUT``, which makes ctypes auto-allocate a single-
element buffer and return it as the function result.  ``protocols.Spi.write_read``
passes the buffer explicitly, resulting in "call takes exactly 6 arguments
(7 given)".

Redefining the three bindings with ``_IN`` for rgRX fixes the mismatch so that
...
def apply() -> None:
    """Patch dwfpy SPI write_read bindings (idempotent)."""
    ...
    logger.info("Applied dwfpy SPI write_read binding patches.")
